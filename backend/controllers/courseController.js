...
import { processLectureVideo } from "../services/transcodeService.js"
...
export const getPublishedCourses = async (req,res) => {
    try {
        ...
        // This endpoint is public (used for course browsing) but is also the
        // frontend's only source of lecture video/assignment URLs for playback.
        // Only reveal those URLs to the course creator, an enrolled student, or
        // for lectures explicitly marked as a free preview - everyone else gets
        // the lecture metadata (title, id) without the playable content.
        const userId = req.userId; // present only if optionalAuth found a valid cookie
        const sanitizedCourses = courses.map((course) => {
            const courseObj = course.toObject();
            const isCreator = Boolean(userId) && course.creator?.toString() === userId;
            const isEnrolled = Boolean(userId) && course.enrolledStudents?.some((id) => id.toString() === userId);
            const hasFullAccess = isCreator || isEnrolled;

            courseObj.lectures = (courseObj.lectures || [])
                // A lecture that's still transcoding (or failed) isn't ready to
                // watch yet - only the creator should see it at all, so they can
                // track progress; students shouldn't see it in the list until ready.
                .filter((lecture) => isCreator || lecture.processingStatus === 'ready')
                .map((lecture) => {
                if (hasFullAccess || lecture.isPreviewFree) {
                    return lecture;
                }
                const { videoUrl, assignmentUrl, assignmentName, ...lockedLecture } = lecture;
...
export const editCourse = async (req,res) => {
    try {
        ...
        if (existingCourse.creator?.toString() !== req.userId) {
        ...
        if (req.file) {
            ...
            if (!thumbnailUrl) {
                return res.status(502).json({message:"Thumbnail upload failed, please try again"});
            }
...
export const removeCourse = async (req, res) => {
  try {
    ...
    if (course.creator?.toString() !== req.userId) {
      return res.status(403).json({ message: "You can only remove your own courses" });
    }
...
export const getCourseLecture = async (req,res) => {
    try {
        ...
        if (!isCreator && !isEnrolled) {
            return res.status(403).json({message:"You do not have access to this course's lectures"})
        }
...
export const editLecture = async (req,res) => {
    try {
        ...
        const owningCourse = await Course.findOne({ lectures: lectureId }).select("creator")
        if (!owningCourse || owningCourse.creator?.toString() !== req.userId) {
        ...
        if(req.files?.videoUrl?.[0]){
            ...
            if (!videoUrl) {
                return res.status(502).json({ message: "Video upload failed, please try again" })
            }
            // videoUrl is the original source - kept as the playable fallback even
            // while (or if) the multi-resolution renditions below are still processing
        ...
        if(req.files?.assignment?.[0]){
            ...
            if (!assignmentUrl) {
                return res.status(502).json({ message: "Assignment upload failed, please try again" })
            }
        ...
        // Kick off transcoding in the background - don't make the teacher's
        // upload request wait for it. The source is already safely on
        // Cloudinary at this point, so this is resumable if the server
        // restarts mid-job (see resumeStuckLectureProcessing in index.js).
        if (shouldStartTranscode) {
            processLectureVideo(lecture._id, owningCourse._id, lecture.videoUrl)
                .catch((err) => console.error(`[editLecture] Transcode kickoff failed for ${lecture._id}:`, err))
        }
...
export const downloadAssignment = async (req, res) => {
    try {
        ...
        const owningCourse = await Course.findOne({ lectures: lectureId }).select("creator enrolledStudents");
        if (!owningCourse) {
            return res.status(404).json({ message: "Lecture not found" });
        }
        const isCreator = owningCourse.creator?.toString() === req.userId;
        const isEnrolled = owningCourse.enrolledStudents?.some((id) => id.toString() === req.userId);
        if (!isCreator && !isEnrolled) {
            return res.status(403).json({ message: "You do not have access to this lecture's assignment" });
        }
...
export const getCreatorById = async (req, res) => {
    try {
        ...
        // Public instructor bio - only expose the fields the course page actually
        // renders, not the full user document (teacher application details, OTP
        // fields, enrolledCourses, etc.)
        const user = await User.findById(userId).select("name email photoUrl description role");
